...
一个为小学生家庭私用设计的本地优先网页应用：孩子先选择并命名自己的宠物，再通过阅读、作业、整理、复盘等学习习惯获得“成长能量”，用能量喂养、清洁、装扮宠物，并陪宠物升级成长。
...
- 第一屏优先服务孩子：选宠物、取名字、看到宠物房间、开始任务、获得即时反馈。
- 成长能量分成“累计能量”和“可用能量”：累计能量用于升级，可用能量用于喂养、清洁和购买装扮。
- 升级同时看累计能量和活跃天数，避免只靠一天刷分冲级。
- 每个任务都有开始按钮、目标用时和实际完成用时，提前完成会有温和加成。
- 家长工具在首页底部保持轻量，只保留导出数据和重新开始。
...
- 9 个规则单测。
- 16 个 Playwright 桌面/移动端 E2E、a11y、视觉冒烟。
